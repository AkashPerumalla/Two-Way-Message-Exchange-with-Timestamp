...
void setup() {
  ...
  timeClient.begin();
  Serial.println("NodeMCU A ready to transmit and receive messages.");
}
...
void loop() {
  ...
  // Check if data is available from HC-12 (i.e., received from NodeMCU B)
  if (hc12.available()) {
    ...
    String timestampedMessage = "NodeMCU A [" + currentTime + " IST]: " + receivedMessage;

    // Output the received message with timestamp to Serial Monitor
    Serial.println("Received: " + timestampedMessage);
  }

  // Check if there is a new message from Serial Monitor to send to NodeMCU B
  if (Serial.available()) {
    String message = Serial.readStringUntil('\n'); // Read input from Serial Monitor
    String currentTime = timeClient.getFormattedTime(); // Get current time in HH:MM:SS format
    String timestampedMessage = "Sonu [" + currentTime + " IST]: " + message;

    // Send the timestamped message to NodeMCU B via HC-12
